...
def load_engine(raw_logs_location, logs_results_location):
    ...
    outlier_datapoints = detect_outlier(df['Elapsed Time'])
...
layout = [
    [sg.Text('Generate Statistics From Baccarat Client Simulator', size=(45, 1), font=("Helvetica", 20),
             text_color='blue')],
    # [sg.Text('IP Address of target Baccarat server:', size=(35, 1)), sg.InputText('10.213.133.65', key='_IP_', do_not_clear=True)],
    # [sg.Text('Port address of Baccarat server:', size=(35, 1)), sg.InputText('4456', key='_PORT_', do_not_clear=True)],
    [sg.Text('Logs location, full path:', size=(40, 1)),
     sg.InputText(r'\\USNVR-W1005006\PublicShare\BaccaratLogs', key='_LOGSDEST', do_not_clear=True)],
    [sg.Text('Statistics and csv file destination:', size=(40, 1)),
     sg.InputText(r'\\USNVR-W1005006\PublicShare\BaccaratLogsCSV', key='_CSVDEST_', do_not_clear=True)],
...
while True:
    ...
    if event == 'Create Stats File':
        # do the stuff here
        load_engine(values['_LOGSDEST'], values['_CSVDEST_'])
